...
def untransform_and_draw_boxes(image: torch.Tensor, boxes, labels, scores, threshold=0.5):
    ...
    for box, label, score in zip(boxes, labels, scores):
        ...
        if score > threshold:
            ...
            if classes[label] == 'person':
                Path(__file__).parent.joinpath("pessoas").mkdir(parents=True, exist_ok=True)
                cv2.imwrite(f'{Path(__file__).parent.joinpath("pessoas").joinpath(f"pessoa{int(time.time())+rand(1,100,1)}.jpeg")}', image[y1:y2, x1:x2])
